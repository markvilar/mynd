...
def load_image_bundle(files: ImageFileBundle) -> ImageBundle:
...
def generate_image_bundle_loaders(
    ...
    pattern: str,
) -> list[ImageBundleLoader]:
    """Generates loaders from a collection of image bundles."""

    file_bundles: list[ImageFileBundle] = _bundle_image_files(
        image_directory,
        range_directory,
        normal_directory,
        pattern,
    ...
    file_bundles: list[ImageFileBundle] = _validate_image_file_bundles(file_bundles)

    file_bundles: list[ImageFileBundle] = sorted(
...
def match_image_files_by_label(
    ...
    frame_files: list[ImageFileBundle] = list()
...
def _validate_image_file_bundles(
    file_bundles: list[ImageFileBundle],
) -> list[ImageFileBundle]:
...
def _bundle_image_files(
    ...
    file_bundles: list[ImageFileBundle] = list()
