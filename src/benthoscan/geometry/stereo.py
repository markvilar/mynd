...
from ..backends.metashape.camera_helpers import SensorPair
...
def compute_rectifying_homographies(sensors: SensorPair) -> RectifyingHomography:
    """
    ...
    resolution: tuple[int, int] = (sensors.master.width, sensors.master.height)
    extrinsics: StereoExtrinsics = compute_stereo_extrinsics(sensors)

    camera_matrix_master: np.ndarray = compute_camera_matrix(sensors.master.calibration)
    camera_matrix_slave: np.ndarray = compute_camera_matrix(sensors.slave.calibration)

    distortion_vector_master: np.ndarray = compute_distortion_vector(sensors.master.calibration)
    distortion_vector_slave: np.ndarray = compute_distortion_vector(sensors.slave.calibration)

    rotation_master, rotation_slave, _, _, _, _, _ = cv2.stereoRectify(
        camera_matrix_master,       # 3x3 master camera matrix
        distortion_vector_master,   # 5x1 master camera distortion
        camera_matrix_slave,        # 3x3 slave camera matrix
        distortion_vector_slave,    # 5x1 slave camera distortion
        resolution,                 # resolution (width, height)
        extrinsics.rotation,        # 3x3 rotation matrix from master to slave
        extrinsics.location,        # 3x1 translation vector from master to slave
        flags=0,
    )
    ...
    homography_master: np.ndarray = rotation_master.dot(np.linalg.inv(camera_matrix_master))
    homography_slave: np.ndarray = rotation_slave.dot(np.linalg.inv(camera_matrix_slave))
...
def compute_rectifying_pixel_maps(
    ...
    distortion_master: np.ndarray = compute_distortion_vector(sensors.master.calibration)
    distortion_slave: np.ndarray = compute_distortion_vector(sensors.slave.calibration)

    extrinsics: StereoExtrinsics = compute_stereo_extrinsics(sensors)

    resolution_master: tuple[int, int] = (sensors.master.width, sensors.master.height)
    resolution_slave: tuple[int, int] = (sensors.slave.width, sensors.slave.height)
    ...
    affine_transform: np.ndarray = _estimate_affine_image_transform(
        camera_matrix_master,               # intrinsic 1
        camera_matrix_slave,                # intrinsic 2
        ...
        distortion_master,                  # distortion 1
        distortion_slave,                   # distortion 2
    ...
    new_camera_master: np.ndarray = affine_transform \
        .dot(homographies.homography_master) \
        .dot(camera_matrix_master) \
        .dot(homographies.rotation.T)
    
    new_camera_slave: np.ndarray = affine_transform \
        .dot(homographies.homography_slave) \
        .dot(camera_matrix_slave) \
        .dot(extrinsics.rotation) \
        .dot(homographies.rotation.T)

    # OpenCV requires the final rotations applied
    R1 = homographies.rotation
    R2 = homographies.rotation.dot(extrinsics.rotation.T)
    ...
    pixel_maps_master: tuple[np.ndarray, np.ndarray] = cv2.initUndistortRectifyMap(
        camera_matrix_master, distortion_master, R1, new_camera_master, desired_resolution, cv2.CV_32FC1
    )
    pixel_maps_slave: tuple[np.ndarray, np.ndarray] = cv2.initUndistortRectifyMap(
        camera_matrix_slave, distortion_slave, R2, new_camera_slave, desired_resolution, cv2.CV_32FC1
    )
