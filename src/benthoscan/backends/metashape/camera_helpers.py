...
from ...geometry.stereo import StereoExtrinsics
...
def get_sensor_pairs(chunk: Metashape.Chunk) -> set[SensorPair]:
    ...
    stereo_sensors: set[SensorPair] = set([
        SensorPair(sensor.master, sensor) for sensor in chunk.sensors if sensor.master != sensor
    ])
...
def get_camera_pairs(chunk: Metashape.Chunk) -> set[CameraPair]:
    ...
    stereo_cameras: set[CameraPair] = set([
        CameraPair(camera.master, camera) for camera in chunk.cameras if camera.master != camera
    ])
...
def compute_camera_matrix(calibration: Metashape.Calibration) -> np.ndarray:
    """Computes the camera matrix for a Metashape calibration."""
...
def compute_stereo_extrinsics(sensors: SensorPair) -> StereoExtrinsics:
    """Compute the extrinsics, i.e. location and rotation, between two Metashape sensors."""

    location: Metashape.Vector = sensors.slave.location * sensors.slave.chunk.transform.scale
